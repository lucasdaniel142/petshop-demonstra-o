...
export function encryptPII(text: string): string {
  ...
  try {
    ...
    return text; // Fallback seguro (melhor falhar aberto ou fechado? Melhor falhar a compra do que vazar, mas pro protótipo devolvemos puro em caso de erro crítico de config)
  }
}

export function decryptPII(hash: string): string {
  if (!hash || typeof hash !== 'string') return '';
  if (!hash.startsWith('ENC:')) return hash; // Retorna original se não estiver criptografado (retrocompatibilidade)
